...
/***************************************************************
 * Create a string from the first item of an F_TextItemsT structure.
 ****************************************************************/
StringT CreateStringFromFirstTextItem(F_TextItemsT textItems)
{
	StringT s1 = NULL, s2 = NULL;
	UIntT i = 0, j = 0;
	BoolT GOTSTRING = False;
	...
	for(i = 0; i < textItems.len; i++)
	{
		if(textItems.val[i].dataType == FTI_String)
		{
			...
			// Find the first non-blank member.
			for (j = 0 ; j < F_StrLen(s1) ; j++)
				if (s1[j] != ' ')
					break;
