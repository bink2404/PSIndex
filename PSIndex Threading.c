...
/* 13/10/03		    Version 2.01 -	Added AutoComplete option.			*/
/*																		*/
/************************************************************************/
...
VoidT testThreads(VoidT *in) ;
VoidT testThreads2(VoidT *in) ;
...
LOGFONT lf;
UCharT			RegValue[800] ;
UCharT			autoComplete[MAXITEMS] [80] ;
IntT			g_autoComplete ;
...
//			F_ApiNotification(FA_Note_PostMouseCommand, True);
			...
			_beginthread(testThreads, 200000, (PVOID)NULL) ;
			_beginthread(testThreads2, 200000, (PVOID)NULL) ;
...
//			F_Printf(NULL, (StringT) "docid = %d\n", docId);
		...
		case ABOUT_CMD:
			if (VERSION == DEMO)
				F_ApiAlert("emDEX client v2.01 Demo\n\nCopyright (c) 2003\n\n\nemail : support@emdex.ca\n\nweb : www.emdex.ca", FF_ALERT_CONTINUE_NOTE);
			else
				F_ApiAlert("emDEX client v2.01\n\nCopyright (c) 2003\n\n\nemail : support@emdex.ca\n\nweb : www.emdex.ca", FF_ALERT_CONTINUE_NOTE);
	...
	while(markerId)
	{
		...
		if ((fmtName != NULL) && (fmtName[0] != '\0'))
		{
			...
			for (i = 0 ; i < F_StrLen(markerName) ; i++)
			{
				...
				{
					if ((tempString[0] != '\0') && (F_StrLen(tempString) > 4) && (F_StrLen(tempString) < 80))
					{
...
//						F_Printf(NULL, (StringT) "autoComplete[%d] = %s\n", autoCount, autoComplete[autoCount]);
			...
			if (tempString[0] != '\0')
			{
				if ((F_StrLen(tempString) > 4) && (F_StrLen(tempString) < 80))
				{
					F_StrCpy(autoComplete[autoCount], tempString) ;
//					F_Printf(NULL, (StringT) "autoComplete[%d] = %s\n", autoCount, autoComplete[autoCount]);
...
VoidT testThreads(VoidT *in)
{
	...
	while (TRUE)
	{
		...
		F_Printf(NULL, (StringT) "2\n");

		if (FindWindow(NULL, szWindowTitle) != NULL)
		{
			F_Printf(NULL, (StringT) "8\n");

			if (updateAllMarkers(docId) != -1)
			{
				F_Printf(NULL, (StringT) "9\n");

				// Update the listbox
				SendMessage(FindWindow(NULL, szWindowTitle), LISTBOX_UPDATE, (WPARAM)0, (LPARAM)0) ;

				F_Printf(NULL, (StringT) "10\n");
			}
		}

		F_Printf(NULL, (StringT) "3\n");
		...
		if ((docId = getContainedInBook(docName)) != 0)
		{
			...
			F_Printf(NULL, (StringT) "4\n");

			if (FindWindow(NULL, szWindowTitle) != NULL)
			{
				F_Printf(NULL, (StringT) "11\n");

				if (updateAllMarkers(docId) == -1)
				{
					F_Printf(NULL, (StringT) "12\n");
				...
				F_Printf(NULL, (StringT) "5\n");

				// Update the listbox
				SendMessage(FindWindow(NULL, szWindowTitle), LISTBOX_UPDATE, (WPARAM)0, (LPARAM)0) ;

				F_Printf(NULL, (StringT) "6\n");
			}
		}
