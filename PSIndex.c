...
/* 23/11/05		    Version 2.01 -	Added AutoComplete option.			*/
/* 17/04/08		    Version 3.00 -	FM8 capability.						*/
/*									Added repeat last marker button.	*/
/*									Enhanced stats						*/
/*																		*/
/************************************************************************/
...
VoidT removeAllTags(StringT inText, StringT outText) ;
...
F_ObjHandleT    editorPopUpId, menuId, bookmenuId, autoCompletePopUpId, autoCompleteCaseId ;
HFONT hf ;
LOGFONT lf;
UCharT			RegValue[800] ;
UCharT			autoComplete[MAXITEMS] [160] ;
IntT			g_autoComplete, g_autoCompleteCase ;
BoolT			g_bEnteringButtonText = False ;

StringT			g_markerName ;
UCharT			g_lastMarkerName[255] ;
...
IntT init;{
	...
	F_ObjHandleT	saveButtonsCmdId, loadButtonsCmdId, autoCompleteOnId, autoCompleteOffId ;
	...
	switch(init){
		case FA_Init_First:
			...
			F_Printf(NULL, (StringT)"Copyright (c) 2008. All rights reserved.\n\n");
			...
			GetPrivateProfileString("Options", "Button9Text", "BoldE&mph", buttons[9].text, 800, "emDEX.ini") ;
			GetPrivateProfileString("Options", "Button9Action", "<Bold><Emphasis>", buttons[9].action, 800, "emDEX.ini") ;
...
//				autoCompleteId= F_ApiDefineAndAddCommand(AUTOCOMPLETE_CMD, menuId, (StringT) "mainautocompleteCmd", (StringT) "AutoComplete", (StringT) "");
				...
				autoCompletePopUpId = F_ApiDefineAndAddMenu(menuId, "mainautocompleteCmd", "AutoComplete");
				autoCompleteOnId= F_ApiDefineAndAddCommand(AUTOCOMPLETE_ON_CMD, autoCompletePopUpId, (StringT) "mainautocompleteOnCmd", (StringT) "On", (StringT) "");
				autoCompleteOffId= F_ApiDefineAndAddCommand(AUTOCOMPLETE_OFF_CMD, autoCompletePopUpId, (StringT) "mainautocompleteOffCmd", (StringT) "Off", (StringT) "");
				autoCompleteCaseId= F_ApiDefineAndAddCommand(AUTOCOMPLETE_CASE_CMD, autoCompletePopUpId, (StringT) "mainautocompleteCaseCmd", (StringT) "Case sensitive", (StringT) "");

				F_ApiSetInt(autoCompletePopUpId, autoCompleteOnId, FP_CanHaveCheckMark, 1);
				F_ApiSetInt(autoCompletePopUpId, autoCompleteOffId, FP_CanHaveCheckMark, 1);
				F_ApiSetInt(autoCompletePopUpId, autoCompleteCaseId, FP_CanHaveCheckMark, 1);

				F_ApiSetInt(autoCompletePopUpId, autoCompleteOnId, FP_CheckMarkIsOn, 0);
				F_ApiSetInt(autoCompletePopUpId, autoCompleteOffId, FP_CheckMarkIsOn, 0);
				F_ApiSetInt(autoCompletePopUpId, autoCompleteCaseId, FP_CheckMarkIsOn, 0);

				GetPrivateProfileString("Options", "AutoComplete", "1", size, 800, "emDEX.ini") ;
				g_autoComplete = atoi(size) ;

				if (g_autoComplete == 1)
					F_ApiSetInt(autoCompletePopUpId, autoCompleteOnId, FP_CheckMarkIsOn, 1);
				else
					F_ApiSetInt(autoCompletePopUpId, autoCompleteOffId, FP_CheckMarkIsOn, 1);

				GetPrivateProfileString("Options", "AutoCompleteCase", "1", size, 800, "emDEX.ini") ;
				g_autoCompleteCase = atoi(size) ;

				if (g_autoCompleteCase == 1)
					F_ApiSetInt(autoCompletePopUpId, autoCompleteCaseId, FP_CheckMarkIsOn, 1);
				else
					F_ApiSetInt(autoCompletePopUpId, autoCompleteCaseId, FP_CheckMarkIsOn, 0);
...
//				autoCompleteId= F_ApiDefineAndAddCommand(AUTOCOMPLETE_CMD, bookmenuId, (StringT) "bookautocompleteCmd", (StringT) "AutoComplete", (StringT) "");
				...
				autoCompletePopUpId = F_ApiDefineAndAddMenu(bookmenuId, "bookautocompleteCmd", "AutoComplete");
				autoCompleteOnId= F_ApiDefineAndAddCommand(AUTOCOMPLETE_ON_CMD, autoCompletePopUpId, (StringT) "bookautocompleteOnCmd", (StringT) "On", (StringT) "");
				autoCompleteOffId= F_ApiDefineAndAddCommand(AUTOCOMPLETE_OFF_CMD, autoCompletePopUpId, (StringT) "bookautocompleteOffCmd", (StringT) "Off", (StringT) "");
				autoCompleteCaseId= F_ApiDefineAndAddCommand(AUTOCOMPLETE_CASE_CMD, autoCompletePopUpId, (StringT) "bookautocompleteCaseCmd", (StringT) "Case sensitive", (StringT) "");
...
//				break ;
		...
		case AUTOCOMPLETE_ON_CMD :
			F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "mainautocompleteOnCmd"), FP_CheckMarkIsOn, True);
			F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "bookautocompleteOnCmd"), FP_CheckMarkIsOn, True);
			F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "mainautocompleteOffCmd"), FP_CheckMarkIsOn, False);
			F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "bookautocompleteOffCmd"), FP_CheckMarkIsOn, False);
		...
		case AUTOCOMPLETE_OFF_CMD :
			F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "mainautocompleteOnCmd"), FP_CheckMarkIsOn, False);
			F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "bookautocompleteOnCmd"), FP_CheckMarkIsOn, False);
			F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "mainautocompleteOffCmd"), FP_CheckMarkIsOn, True);
			F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "bookautocompleteOffCmd"), FP_CheckMarkIsOn, True);
		...
		case AUTOCOMPLETE_CASE_CMD :
			if (g_autoCompleteCase == 1)
			{
				F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "mainautocompleteCaseCmd"), FP_CheckMarkIsOn, False);
				F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "bookautocompleteCaseCmd"), FP_CheckMarkIsOn, False);

				WritePrivateProfileString("Options", "AutoCompleteCase", "0", "emDEX.ini");
				g_autoCompleteCase = 0 ;
			}
			else
			{
				F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "mainautocompleteCaseCmd"), FP_CheckMarkIsOn, True);
				F_ApiSetInt(autoCompletePopUpId, F_ApiGetNamedObject(FV_SessionId, FO_Command, "bookautocompleteCaseCmd"), FP_CheckMarkIsOn, True);

				WritePrivateProfileString("Options", "AutoCompleteCase", "1", "emDEX.ini");
				g_autoCompleteCase = 1 ;
			}
...
//			F_Printf(NULL, (StringT) "docid = %d\n", docId);
		...
		case ABOUT_CMD:
			if (VERSION == DEMO)
				F_ApiAlert("emDEX client v3.00 Demo\n\nCopyright (c) 2008\n\n\nemail : info@emdex.ca\n\nweb : www.emdex.ca", FF_ALERT_CONTINUE_NOTE);
			else
				F_ApiAlert("emDEX client v3.00\n\nCopyright (c) 2008\n\n\nemail : info@emdex.ca\n\nweb : www.emdex.ca", FF_ALERT_CONTINUE_NOTE);
...
/*				if (VERSION == DEMO)
				{
					if (demoCount > 10)
					{
						F_ApiAlert((StringT) "The demo version of emDEX is limited to 10 marker edits per session.\n\nYou can order the full version at www.emdex.ca", FF_ALERT_CONTINUE_WARN);
						return ;
					}
					else
					{
...
//					F_Printf(NULL, (StringT) "start id = %d\n", markerId) ;
				...
				F_StrCpy(g_lastMarkerName, rangeText) ;
...
//					F_Printf(NULL, (StringT) "end id = %d\n", markerId) ;
			...
			case 19 :
			case 20 :
				if ((VERSION == DEMO) && (itemNum > 15))
				{
					F_ApiAlert((StringT) "You are running the demo version of emDEX.\n\nOnly the first row of editor buttons is available.\n\nYou can order the full version at www.emdex.ca", FF_ALERT_CONTINUE_WARN);
				}
				else
				{
					hEditWindow = FindWindow(NULL, "emDEX Range Marker") ;
					...
					// Don't try to autocomplete when entering button text
					g_bEnteringButtonText = True ;
			...
			case 23:
				...
				SetFocus(hTextEntry) ;
				SetForegroundWindow(hTextEntry) ;
				F_ApiSetString(dialogId, rangeTextId, FP_Text, g_lastMarkerName);
...
VoidT doAutoComplete()
{
	...
	F_ObjHandleT docId ;
	IntT i, j, k, l, len ;
	static UCharT	tempText[800] ;
	static UCharT	tempText2[800] ;
	UCharT	autoCompleteText[800] ;

	// Don't try to autocomplete when entering button text
	if (g_bEnteringButtonText == True)
		return ;
...
//	F_Printf(NULL, (StringT) "rangeText = %s, start = %d, end = %d\n", rangeText, start, end);
	...
	for (i = start-1 ; i > 0 ; i--)
	{
/*		if (!isalnum(rangeText[i]))
			break ; */
	}

	for (j = start ; j < F_StrLen(rangeText) ; j++)
	{
/*		if (!isalnum(rangeText[j]))
			break ; */
	}
	...
	len = F_StrLen(tempText) ;

	tempText[0] = '\0' ;
	F_StrCpyN(tempText, &rangeText[i], j-i) ;

	// Don't autocomplete if text being deleted
	if ((j-i) <= len)
		return ;

//	F_Printf(NULL, (StringT) "rangeText = %s, start = %d, end = %d, i = %d, j = %d, tempText = %s\n", rangeText, start, end, i, j, tempText);

	// Get the text after the final ';' character
	for (l = F_StrLen(tempText) ; l >= 0 ; l--)
	{
		if ((tempText[l] == ';') && (isEscapeSequence(tempText, l) == False))
		{
			F_StrCpy(tempText, &tempText[l+1]) ;
			break ;
		}
	}

	F_StrStripLeadingSpaces(tempText) ;
	removeAllTags(tempText, tempText2) ;

//	F_Printf(NULL, (StringT) "tempText = <%s>\n", tempText);

	for (k = 0 ; autoComplete[k] [0] != '\0', k < MAXITEMS ; k++)
	{
		if ((F_StrLen(tempText2) >= 2) && 
			(((F_StrICmpN(tempText2, autoComplete[k], F_StrLen(tempText2)) == 0) && (g_autoCompleteCase == 0)) || 
			((F_StrCmpN(tempText2, autoComplete[k], F_StrLen(tempText2)) == 0) && (g_autoCompleteCase == 1))) &&
			(F_StrICmp(tempText2, autoComplete[k]) != 0))
		{
			completeCount = 1 ;

			F_StrCpyN(autoCompleteText, rangeText, j) ;
			F_StrCat(autoCompleteText, &autoComplete[k] [F_StrLen(tempText2)]) ;
...
//			F_Printf(NULL, (StringT) "autoCompleteText = %s\n", autoCompleteText);
			...
			SendMessage(hTextEntry, EM_SETSEL, (WPARAM)j + (F_StrLen(autoComplete[k]) - F_StrLen(tempText2)) - 1, (LPARAM)end) ;
...
VoidT loadAutoCompleteText(F_ObjHandleT docId)
{
	F_ObjHandleT markerId;
	StringT markerName ;
	UCharT tempString1[800] ;
	UCharT tempString2[800] ;
	IntT i, j, k, l, autoCount ;
	IntT begin, end ;
	StringT fmtName;
	F_ObjHandleT markerTypeId ;
	BoolT isLooped = False ;

	// Get all markers and update the autocomplete list.
	...
	if (autoComplete[MAXITEMS-1] [0] == '\0')
	{
		for (autoCount = 0 ; autoComplete[autoCount] [0] != '\0' ; autoCount++) ;
	}
	else
	{
		for (autoCount = 0 ; F_StrCmp(autoComplete[autoCount], (StringT)"xxx!") != 0 ; autoCount++) ;

		isLooped = True ;
	}

//	F_Printf(NULL, (StringT) "autoCount = %d\n", autoCount);
	...
	while(markerId)
	{
		...
		if ((fmtName != NULL) && (fmtName[0] != '\0'))
		{
			...
			if (markerName[0] != '\0')
			{
				if ((F_StrLen(markerName) >= 2) && (F_StrLen(markerName) <= 160))
				{
					begin = end = 0 ;

					// Get any multiple text strings separated by ';'
					for (l = 0 ; l < F_StrLen(markerName) ; l++)
					{
						if ((markerName[l] == ';') && (isEscapeSequence(markerName, l) == False))
						{
							end = l+1 ;
							tempString1[0] = '\0' ;
							tempString2[0] = '\0' ;
							F_StrCpyN(tempString1, &markerName[begin], end-begin) ;
							tempString1[end-begin] = '\0' ;

							F_StrStripLeadingSpaces(tempString1) ;
							removeAllTags(tempString1, tempString2) ;

							F_StrCpy(autoComplete[autoCount], tempString2) ;
//							F_Printf(NULL, (StringT) "autoComplete[%d] = %s\n", autoCount, autoComplete[autoCount]);

							if (autoCount == MAXITEMS)
							{
								isLooped = True ;
								autoCount = 0 ;
							}
							...
							if (isLooped == True)
								F_StrCpy(autoComplete[autoCount], "xxx!") ;
							else
								autoComplete[autoCount] [0] = '\0' ;
...
VoidT addAutoCompleteText(StringT inString)
{
	...
	// Add the text into the autocomplete list
	...
	if (inString[0] != '\0')
	{
		if ((F_StrLen(inString) >= 2) && (F_StrLen(inString) <= 160))
		{
			begin = end = 0 ;

			// Get any multiple text strings separated by ';'
			for (l = 0 ; l < F_StrLen(inString) ; l++)
			{
				if ((inString[l] == ';') && (isEscapeSequence(inString, l) == False))
				{
					end = l+1 ;
					tempString1[0] = '\0' ;
					tempString2[0] = '\0' ;
					F_StrCpyN(tempString1, &inString[begin], end-begin) ;
					tempString1[end-begin] = '\0' ;
...
//						F_Printf(NULL, (StringT) "NONBLANK tempMarkerName = %s\n", tempMarkerName);
						...
						// FM hangs when sending a string of "protocols"
						if (F_StrICmp(tempMarkerName, (StringT)"protocols") == 0)
						{
							F_StrCat(tempMarkerName, (StringT)" ") ;
						}
				...
				if (!((markerName[F_StrLen(markerName)-1] == ';') && 
					(isEscapeSequence(markerName, F_StrLen(markerName)-1) == False)))
				{
...
//				F_Printf(NULL, (StringT) "temp = %s\n", temp);
//				F_Printf(NULL, (StringT) "title = %s\n", szWindowTitle);
...
/*	fcodes[0] = CSR_BOE ;
	...
	if (F_StrLen(g_markerName) > 0)
	{
		F_StrCpy(g_lastMarkerName, g_markerName) ;
	}
...
VoidT removeAllTags(StringT inText, StringT outText)
{
	...
	for (i = 0, j = 0 ; i < (INT)F_StrLen(inText) ; i++)
	{
		...
		{
			while ((inText[i] != '>') && (isEscapeSequence(inText, i) == False) && (i < (INT)F_StrLen(inText)))
			{
				i++ ;
			}
		}
...
VoidT openEditorWindow(StringT title)
{
	...
	if ((atoi(posX) > GetSystemMetrics(SM_CXSCREEN)) || (atoi(posX) < 0))
	{
		F_StrCpy(posX, "100") ;
	}

	if ((atoi(posY) > GetSystemMetrics(SM_CYSCREEN)) || (atoi(posY) < 0))
	{
		F_StrCpy(posY, "100") ;
	}
...
/*			for (i = 0; i < validEntries.len; i++)
	...
	if (g_CurrentEditor == MARKER_WINDOW)
	{
		...
		if (textItems.len > 0)
		{
			...
			if (F_StrLen(string) > 0)
			{
				F_StrCpy(g_lastMarkerName, string) ;
			}
...
/*		else
	...
	if ((g_CurrentEditor == RANGE_WINDOW) && (hEditWindow != NULL))
	{
		if (F_StrLen(string) > 0)
		{
			F_StrCpy(g_lastMarkerName, string) ;
...
VoidT setEditorSize(IntT size)
{
	...
	switch (size)
	{
		case SMALL :
			...
			SetWindowPos(GetDlgItem(hEditWindow, 123), HWND_TOP, 105, 188, 0, 0, SWP_NOSIZE | SWP_NOZORDER) ;
		...
		case MEDIUM :
			...
			SetWindowPos(GetDlgItem(hEditWindow, 123), HWND_TOP, 129, 247, 0, 0, SWP_NOSIZE | SWP_NOZORDER) ;
		...
		case LARGE :
			...
			SetWindowPos(GetDlgItem(hEditWindow, 123), HWND_TOP, 153, 297, 0, 0, SWP_NOSIZE | SWP_NOZORDER) ;
